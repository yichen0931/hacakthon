            ...
            <div className="overflow-y-auto">
                ...
                {!loading && data && (
                    data.map((item) => {
                        ...
                        return(<VendorCard key={item.VendorID} imgsrc={backgroundImage} name={item.VendorName} startTime={formatTime(item.DiscountStart)} endTime={formatTime(item.DiscountEnd)} address={item.Address} link={`/customer/deals/${item.VendorID}`}>
                        </VendorCard>)
                    })
                )}
            </div>
