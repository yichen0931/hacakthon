...
func GetSQL(db *sql.DB, tableName string) (result interface{}, err error) {
	...
	if result == nil {
		return nil, fmt.Errorf("GetTable: Unknown table %s", tableName)
	}
...
func DeleteSQL(db *sql.DB, tableName string, idField string, idValue interface{}) error {
	query := fmt.Sprintf("DELETE FROM %s WHERE %s='%s'", tableName, idField, idValue)
	fmt.Println(query)
	_, err := db.Exec(query)
	if err != nil {
		return fmt.Errorf("error executing query: %w", err)
